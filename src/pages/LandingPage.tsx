...
const steps = [
  { num: "01", title: "Создай аккаунт",    desc: "Регистрация за 30 секунд — никаких лишних полей", color: "text-swim" },
  { num: "02", title: "Подключи Strava",   desc: "Все прошлые и будущие тренировки появятся автоматически", color: "text-bike" },
  { num: "03", title: "Анализируй прогресс", desc: "Смотри динамику, планируй пиковую форму к гонке", color: "text-run" },
];
  ...
  return (
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-3.5">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link to="/auth/register">
              <Button size="sm" className="gap-1.5 shadow-[0_0_20px_hsl(var(--primary)/0.3)]">
      ...
      <section className="relative px-6 pt-20 pb-24 text-center lg:pt-24 lg:pb-32 min-h-screen flex flex-col justify-center">
        ...
        <div className="relative z-[2] mx-auto max-w-5xl">
          ...
          >
            ...
            <span className="text-white/60 font-medium"> финишируй </span>
            быстрее
          </motion.h1>
          ...
          >
            <Link to="/auth/register">
              <Button size="lg" className="gap-2 px-8 h-12 text-base shadow-[0_0_30px_hsl(var(--primary)/0.4)]">
                Начать бесплатно <ArrowRight className="h-4 w-4" />
              </Button>
            </Link>
            <Link to="/auth/login">
              <Button variant="outline" size="lg" className="h-12 text-base px-8 border-white/20 bg-white/5 backdrop-blur-sm hover:bg-white/10">
                Уже есть аккаунт
              </Button>
            </Link>
          </motion.div>
          ...
          >
            {sports.map((s) => (
              <div
                key={s.label}
                className={cn(
                  "flex flex-col items-center gap-3 rounded-2xl border p-5 backdrop-blur-sm transition-all duration-200 cursor-default",
                  s.bg
                )}
              >
      ...
      <section
        ...
        style={{ backgroundImage: "linear-gradient(to bottom, hsl(220 18% 8% / 0.78), hsl(220 18% 8% / 0.78)), url('/bg2.jpg')", backgroundSize: "cover", backgroundPosition: "center", backgroundAttachment: "scroll" }}
      >
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {features.map((f, i) => (
              <motion.div
                key={f.title}
                className="group rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md p-6 transition-all duration-200 hover:bg-white/10 hover:border-white/20 hover:-translate-y-1"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                transition={{ delay: i * 0.07, duration: 0.45 }}
                viewport={{ once: true }}
              >
                <div className={cn("inline-flex h-11 w-11 items-center justify-center rounded-xl border mb-4", f.accent)}>
      ...
      <section
        className="py-28"
        style={{ backgroundImage: "linear-gradient(to bottom, hsl(220 18% 8% / 0.78), hsl(220 18% 8% / 0.78)), url('/bg7.jpg')", backgroundSize: "cover", backgroundPosition: "center", backgroundAttachment: "scroll" }}
      >
        <div className="mx-auto max-w-4xl px-6">
          ...
          <div className="grid gap-8 md:grid-cols-3">
            {steps.map((s, i) => (
              <motion.div
                key={s.num}
                className="relative rounded-2xl border border-border/50 bg-card p-7"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                transition={{ delay: i * 0.12, duration: 0.45 }}
                viewport={{ once: true }}
              >
                <span className={cn("font-display text-4xl font-bold block mb-3", s.color)}>{s.num}</span>
