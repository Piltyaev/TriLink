...
serve(async (req) => {
  ...
  try {
    if (Deno.env.get('DEBUG')) console.log("[strava-oauth] invoked");
    const { code, userId } = await req.json();
    if (!code || !userId) throw new Error("Missing code or userId");
    if (Deno.env.get('DEBUG')) console.log("[strava-oauth] code received for userId:", userId);

    const clientId = Deno.env.get("STRAVA_CLIENT_ID");
    const clientSecret = Deno.env.get("STRAVA_CLIENT_SECRET");
    if (!clientId || !clientSecret) throw new Error("STRAVA_CLIENT_ID or STRAVA_CLIENT_SECRET not set");
    if (Deno.env.get('DEBUG')) console.log("[strava-oauth] secrets OK, clientId:", clientId);
    ...
    if (Deno.env.get('DEBUG')) console.log("[strava-oauth] Strava token response status:", tokenRes.status, "body:", JSON.stringify(tokenData));
